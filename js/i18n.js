...
(function (TTV) {
  ...
  var DICT = {
    de: {
      ...
      seqIntro: 'Tippe die Schläge von <strong>Spieler A</strong> als Sequenz – ein Schlag pro Zeile, oder mit <code>-&gt;</code>, Komma bzw. <code>und</code> getrennt; eine Gruppe wiederholen mit <code>2x …</code>. <strong>Spieler B</strong> wird automatisch ergänzt (Topspin→Block, Konter→Konter, Block→Topspin …) – optional unten überschreibbar.',
    ...
    en: {
      ...
      seqIntro: 'Type <strong>Player A</strong>’s strokes as a sequence – one per line, or separated by <code>-&gt;</code>, a comma or <code>and</code>; repeat a group with <code>2x …</code>. <strong>Player B</strong> is filled in automatically (topspin→block, counter→counter, block→topspin …) – optionally overridden below.',
